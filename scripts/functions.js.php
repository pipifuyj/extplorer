...
/**
 * @version $Id: functions.js.php 150 2009-06-27 20:10:10Z soeren $
 * @package eXtplorer
...
function openActionDialog( caller, action ) {
	...
	switch( action ) {
		...
		case 'extract':
			...
			break;
		case 'download':
			var path  = '<?php echo basename($GLOBALS['script_name']) ?>?option=com_extplorer&action=download&item='+ encodeURIComponent(ext_itemgrid.getSelectionModel().getSelected().get('name')) + '&dir=' + encodeURIComponent( datastore.directory );
