...
/**
 * @version $Id: functions.php 135 2009-01-27 21:57:15Z ryu_ms $
 * @package eXtplorer
...
//------------------------------------------------------------------------------
 function get_abs_item_by_user($dir, $item) {            // get absolute file+path+user
        if( is_array( $item )) {
                // FTP Mode
                $abs_item = '/' . get_abs_dir($dir)."/".$item['name'];
                if( get_is_dir($item)) $abs_item.='/';
        ...
        return extPathName( get_abs_dir($dir)."/".date('Y-m-d_H:i:s', strtotime('now')).'_'.$GLOBALS['__SESSION']["s_user"].'_'.$item );
 }
